...
export default function ChartsView({ properties, transactions }: ChartsViewProps) {
  ...
  // Per-property bar chart (simple CSS bars)
            ...
            return (
              <div key={p.id}>
                ...
                <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                  <div className="bg-emerald-500 h-3 rounded-full transition-all" style={{ width: `${(p.rentValue / maxRent) * 100}%` }} />
                  {received > 0 && (
                    <div className="bg-emerald-700 h-3 rounded-full -mt-3 transition-all" style={{ width: `${(received / maxRent) * 100}%` }} />
                  )}
                </div>
              </div>
            );
